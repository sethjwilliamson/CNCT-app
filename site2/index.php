...
<div id="navbar">
  ...
  <a style="position:absolute; width: 20%; margin-left: 40%; margin-right: 40%;; font-weight:bold; " href="#">CNCT</a>
</div>
            ...
            <div class="col-sm-3">
                <div class="panel panel-default" style="background-color:#f9f9f9; padding:15px">
                    ...
                    <form>
                        ...
                        <input type="text" placeholder="Start Date - 1111/11/11" class="form-control" id="startDate">
                        <input type="text" placeholder="End Date - 2019/03/29" class="form-control" id="endDate">
...
<script type="text/javascript">
    ...
    // When document is fully loaded, run onSubmit()
    $(document).ready(function () {
        onSubmit();
    });

    // When user is almost to bottom, run getData()
    ...
    function authenticateClicked() {
        ...
        <div class="row">
            ...
            <a onclick="authenticateYoutubeClicked()" href="https://accounts.google.com/o/oauth2/auth?response_type=code&access_type=offline&client_id=750014165959-imptae4m45hv4kvjid7gqmu6ab3guvoj.apps.googleusercontent.com&redirect_uri=urn%3Aietf%3Awg%3Aoauth%3A2.0%3Aoob&state&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.readonly&approval_prompt=auto" target="_blank"><img src="/site2/Images/youtubelogo.jpg" width="50" height="50" /></a>
        </div>
        `);
    }
    ...
    function onYoutubeAuthenticationClick() {
        $.ajax({
            url: '/site2/Scripts/Youtube/Youtube.php',
            type: 'POST',
            dataType: 'text',
            data: {
                code: document.getElementById("userInput").value,
                userid: username
            },
            success: function (response) {
                alert("worked ???");
    ...
    // Gets posts from database and adds it to the feed page
    function getData() {
        // When it has reached the final post fitting the filter criteria, ignore
        if (reachedMax) {
            return;
        }

        // Sends HTTP request to getData.php to retrieve necessary post info fitting query
        $.ajax({
